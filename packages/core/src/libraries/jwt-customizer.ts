...
export class CreateJwtCustomizerLibrary {
   ...
   */
  async deployJwtCustomizerScript<T extends LogtoJwtTokenKey>(payload: {
    key: T;
    ...
    const [client, jwtCustomizers] = await Promise.all([
      ...
      this.logtoConfigs.getJwtCustomizers(),
    ]);
  ...
  async undeployJwtCustomizerScript<T extends LogtoJwtTokenKey>(key: T) {
    if (!EnvSet.values.isCloud) {
      return;
    }
